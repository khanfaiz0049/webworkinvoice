<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto" x-data='{
        ...
        selectedServices: {{ json_encode(old('services', $project->services ?? [])) }},
        ...
        init() {
            ...
            this.$watch("startDate", () => this.updateRenewalDate());
            this.$watch("renewalPeriod", () => this.updateRenewalDate());
        },
        ...
        updateRenewalDate() {
            ...
            try {
                ...
                this.renewalDate = `${yyyy}-${mm}-${dd}`;
            } catch (e) {
                console.error(e);
            }
        }
    }'>
        <form action="{{ route('projects.update', $project) }}" method="POST" class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            @csrf
            @method('PUT')
            <div class="px-4 sm:px-6 lg:px-10 py-8 border-b border-slate-50 bg-slate-50/50">
                <h3 class="font-black text-2xl uppercase tracking-tighter italic text-slate-900">Edit Project</h3>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Update project details and renewal schedule</p>
            </div>
            ...
            <div class="p-4 sm:p-6 lg:p-10 space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $project->name) }}" required class="w-full bg-slate-50 border-slate-200 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-[#0055a4] focus:border-[#0055a4] transition-all font-bold text-slate-900 placeholder-slate-300" placeholder="e.g. Website Development">
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="number" name="amount" value="{{ old('amount', $project->amount) }}" required step="0.01" min="0" class="w-full bg-slate-50 border-slate-200 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-[#0055a4] focus:border-[#0055a4] transition-all font-bold text-slate-900 placeholder-slate-300" placeholder="0.00">
                    </div>
                    <div class="space-y-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Renewal Date</label>
                        <input type="date" name="renewal_date" x-model="renewalDate" required class="w-full bg-slate-50 border-slate-200 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-[#0055a4] focus:border-[#0055a4] transition-all font-bold text-slate-900">
                    </div>
                    <div class="space-y-2">
                        ...
                        <select name="renewal_period" x-model="renewalPeriod" required class="w-full bg-slate-50 border-slate-200 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-[#0055a4] focus:border-[#0055a4] transition-all font-bold text-slate-900">
                            @foreach(\App\Models\Project::RENEWAL_PERIODS as $value => $label)
                                <option value="{{ $value }}" {{ old('renewal_period', $project->renewal_period)===$value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="notes" rows="3" class="w-full bg-slate-50 border-slate-200 rounded-2xl px-5 py-4 focus:ring-2 focus:ring-[#0055a4] focus:border-[#0055a4] transition-all font-bold text-slate-900 placeholder-slate-300 resize-none" placeholder="Any additional notes...">{{ old('notes', $project->notes) }}</textarea>
                </div>
            </div>
            ...
            <div class="px-4 sm:px-6 lg:px-10 py-8 bg-slate-50/50 border-t border-slate-50 flex items-center justify-between">
                ...
                <button type="submit" class="bg-[#0055a4] hover:bg-[#004482] text-white font-black py-4 px-12 rounded-2xl transition-all duration-200 shadow-xl shadow-blue-500/20 active:scale-95 text-xs uppercase tracking-widest">Update Project</button>
            </div>
        </form>
    </div>
</x-app-layout>
