<x-app-layout>
    ...
    <div class="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden w-full">
        <div class="px-4 sm:px-6 lg:px-10 py-6 sm:py-8 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50 w-full">
            ...
            <a href="{{ route('companies.create') }}" class="w-full sm:w-auto bg-[#d32d27] hover:bg-[#b21f24] text-white font-black py-3 px-8 rounded-xl transition-all duration-200 shadow-lg shadow-red-500/20 active:scale-95 text-xs uppercase tracking-widest flex items-center justify-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] bg-white">
                        <th class="px-4 sm:px-6 lg:px-10 py-6">Company Name</th>
                        <th class="px-4 sm:px-6 lg:px-10 py-6">Email</th>
                        <th class="px-4 sm:px-6 lg:px-10 py-6">Phone</th>
                        <th class="px-4 sm:px-6 lg:px-10 py-6">GSTIN</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($companies as $company)
                        <tr class="group hover:bg-blue-50/30 transition-colors">
                            <td class="px-4 sm:px-6 lg:px-10 py-6 font-bold text-slate-900 uppercase tracking-tight italic">{{ $company->name }}</td>
                            <td class="px-4 sm:px-6 lg:px-10 py-6 text-sm text-slate-500 font-medium">{{ $company->email }}</td>
                            <td class="px-4 sm:px-6 lg:px-10 py-6 text-sm text-slate-500 font-medium">{{ $company->phone }}</td>
                            <td class="px-4 sm:px-6 lg:px-10 py-6 text-xs font-black text-[#0055a4] uppercase tracking-widest">{{ $company->gst_number ?? 'N/A' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 sm:px-6 lg:px-10 py-20 text-center">
