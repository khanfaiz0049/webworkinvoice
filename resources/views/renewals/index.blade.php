<x-app-layout>
    ...
    <div class="bg-white rounded-2xl sm:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden w-full">
        <div class="px-4 sm:px-6 lg:px-10 py-6 sm:py-8 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50 w-full">
            ...
            <a href="{{ route('renewals.create') }}" class="w-full sm:w-auto bg-[#d32d27] hover:bg-[#b21f24] text-white font-black py-3 px-8 rounded-xl transition-all duration-200 shadow-lg shadow-red-500/20 active:scale-95 text-xs uppercase tracking-widest flex items-center justify-center gap-2 shrink-0">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] bg-white">
                        ...
                        <th class="px-4 sm:px-6 lg:px-10 py-6">Due Date</th>
                        <th class="px-4 sm:px-6 lg:px-10 py-6">Frequency</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($renewals as $renewal)
                        <tr class="group hover:bg-red-50/30 transition-colors">
                            <td class="px-4 sm:px-6 lg:px-10 py-6 font-bold text-slate-900 uppercase tracking-tight italic">{{ $renewal->name }}</td>
                            ...
                            <td class="px-4 sm:px-6 lg:px-10 py-6 text-sm {{ \Carbon\Carbon::parse($renewal->due_date)->isPast() ? 'text-red-500 font-black' : 'text-slate-500 font-medium' }}">
